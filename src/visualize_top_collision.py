...
def generate_final_report():
    # Read the final merged collision-track report produced by the pipeline
    ...
    # Exit early if the final report exists but contains no collision rows
    if df.empty:
        print("No collision tracks found in the data.")
        return

    # The combined final file is already sorted by severity, so the
    # first pair_id represents the highest-ranked collision event
    ...
    # Prefer vessel names when present otherwise use MMSI labels
    ...
    # Use the nearest reported AIS point to the interpolated collision time as
    # the "impact" location for each vessel
    ...
    # Print a compact console summary of the top-ranked collision event.
    print("\n" + "=" * 50)
    print("Top collision report")
    print("=" * 50)
